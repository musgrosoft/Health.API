...
//        private static string theAccessToken;
        ...
        private FitbitActivityClient _client;

        public FitbitService(IConfig config, ILogger logger, string accessToken, ICalendar calendar, HttpClient httpClient, FitbitActivityClient client)
        {
        ...
        {
            var fitbitDailyActivity = await _client.GetActivity(date);
        ...
        {
            var heartSummaries = await _client.GetMonthOfHeartSummaries(dateTime);
        ...
        private async Task<IEnumerable<HeartRateZoneSummary>> GetMonthOfHeartZones(DateTime date)
        {
            ...
            var heartSummaries = await _client.GetMonthOfHeartSummaries(date);
